...
interface DashboardInterface {
  mode: boolean;
  email?: string; // Permitir opcional por seguridad
}

export function DashboardPage({ mode, email }: DashboardInterface) {
  ...
  useEffect(() => {
    ...
    setTasks([
      { id: "t1", title: "Diseñar navbar", description: "", status: "in_progress", priority: "medium", projectId: "1", assignedTo: "jhelcy", estimatedHours: 5, actualHours: 2, dueDate: "2024-07-21", createdAt: "2024-07-01" },
      { id: "t2", title: "Setup MongoDB", description: "", status: "todo", priority: "high", projectId: "2", assignedTo: "jhelcy", estimatedHours: 3, actualHours: 0, dueDate: "2024-07-25", createdAt: "2024-07-15" }
    ]);
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="p-4 bg-white shadow rounded">
            ...
            <ul className="list-disc pl-5 space-y-1">
              {projects.map(proj => (
                <li key={proj.id}>
                  {proj.name} - <span className="text-sm text-gray-500">{proj.status}</span>
                </li>
              ))}
            </ul>
          </div>
